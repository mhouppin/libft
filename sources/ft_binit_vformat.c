...
static int int_set_precision(ByteArray *dst, ParseFormat *pf)
{
    ...
    return ft_bfill_front(dst, '0', (size_t)pf->precision - size);
}

static int int_sharp(ByteArray *dst, ParseFormat *pf)
{
    if (dst->size == 0 && pf->specifier == 'o')
        return ft_bpush_back(dst, '0');

    if (pf->specifier != 'p' && (dst->size == 0 || (*dst->data == '0' && dst->size == 1)))
        return 0;

    return (pf->specifier == 'o') ? ft_bpush_front(dst, '0') : ft_bpush_front_range(dst, "0x", 2);
}
...
static int int_zero_padding(ByteArray *dst, ParseFormat *pf, bool negative)
{
    ...
    return ft_bfill_front(dst, '0', (size_t)paddingSize);
}
...
{
    (void)pf;
    ft_bpush_back(dst, va_arg(ap, int));
    return 0;
}
...
static int print_int(ByteArray *dst, ParseFormat *pf, va_list ap)
{
    ...
    if (value < 0)
        return ft_bpush_front(dst, '-');

    else if (pf->flags & PF_Plus)
        return ft_bpush_front(dst, '+');

    else
        return (pf->flags & PF_Space) ? ft_bpush_front(dst, ' ') : 0;
}
...
static int print_str(ByteArray *dst, ParseFormat *pf, va_list ap)
{
    ...
    return ft_bpush_back_range(dst, str, size);
}
...
static const char *print_conv(ByteArray *b, const char *fmt, va_list ap)
{
    ...
    {
        if (pf.specifier != '\0' && ft_bpush_back(&conversion, pf.specifier))
        {
            ft_bdestroy(&conversion);
            return NULL;
        }
    }
    ...
    if (ft_bpush_back_range(b, conversion.data, conversionSize))
    {
        ft_bdestroy(&conversion);
        return NULL;
    }
    ...
    if (conversionSize < (size_t)pf.width)
    {
        if (pf.flags & PF_Minus)
        {
            if (ft_bfill_back(b, fillerByte, (size_t)pf.width - conversionSize))
                return NULL;
        }
        else if (ft_bfill(b, b->size - conversionSize, fillerByte, (size_t)pf.width - conversionSize))
            return NULL;
    }
...
int ft_binit_vformat(ByteArray *b, const char *fmt, va_list ap)
{
    ...
    while (*fmt)
    {
        ...
        if (percent != NULL)
        {
            if (ft_bpush_back_range(b, fmt, percent - fmt))
            {
                ft_bdestroy(b);
                return -1;
            }
        ...
        {
            if (ft_bpush_back_range(b, fmt, ft_strlen(fmt)))
            {
                ft_bdestroy(b);
                return -1;
            }
